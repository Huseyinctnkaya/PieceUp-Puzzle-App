...
describe("fetchConfig", () => {
  it("returns the config object from the response", async () => {
    global.fetch.mockResolvedValue({
      ok: true,
      json: async () => ({ config: { pieceCount: 9 } }),
    });
...
describe("fetchStatus", () => {
  ...
  it("throws when the server returns a non-ok response", async () => {
    global.fetch.mockResolvedValue({ ok: false, json: async () => ({}) });
    await expect(fetchStatus("device:abc")).rejects.toThrow(
      "status_fetch_failed",
    );
  });
});
...
describe("submitCompletion", () => {
  it("returns the discount code on success", async () => {
    global.fetch.mockResolvedValue({
      ok: true,
      json: async () => ({ discountCode: "PIECEUP-ABC123" }),
    });
  ...
  it("throws when the server reports already_played", async () => {
    global.fetch.mockResolvedValue({
      ok: false,
      json: async () => ({ error: "already_played" }),
    });
    await expect(submitCompletion("device:abc")).rejects.toThrow(
      "already_played",
    );
  });
});
